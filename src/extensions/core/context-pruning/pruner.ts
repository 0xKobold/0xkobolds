/**
 * Context Pruning - Message Pruner
 * 
 * Adapted from OpenClaw - intelligent message pruning to fit context windows
 */
...
// We currently skip pruning tool results that contain images
...
function makeToolPrunablePredicate(match: { allow?: string[]; deny?: string[] }): (toolName: string) => boolean {
  const allowList = match.allow ?? [];
  const denyList = match.deny ?? [];
  const hasAllowList = allowList.length > 0;
  
  return (toolName: string) => {
    if (denyList.length > 0) {
      for (const d of denyList) {
        if (toolName === d || (d.includes("*") && new RegExp("^" + d.replace(/\*/g, ".*") + "$").test(toolName))) {
          return false;
        }
      }
    }
    ...
    for (const a of allowList) {
      if (toolName === a || (a.includes("*") && new RegExp("^" + a.replace(/\*/g, ".*") + "$").test(toolName))) {
        return true;
      }
    }
    return false;
...
export function pruneContextMessages(params: {
  ...
  // Protect initial identity reads (SOUL.md, USER.md, etc.)
