 ...
 * 
 * Adapted from OpenClaw - manages context window limits intelligently
 */
...
function parseDurationMs(input: string, { defaultUnit = "m" }: { defaultUnit?: "ms" | "s" | "m" | "h" | "d" } = {}): number {
  const trimmed = input.trim();
  const match = trimmed.match(/^([0-9.]+)\s*([a-z]*)$/i);
  if (!match) {
  ...
  let unit = match[2].toLowerCase() || defaultUnit;
  switch (unit) {
    case "ms":
      return value;
    case "s":
    ...
    default:
      throw new Error(`Unknown time unit: "${unit}" in "${input}"`);
  }
}
