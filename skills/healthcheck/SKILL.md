---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Kobold deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Kobold cron scheduling for periodic checks, or version status checks on a machine running Kobold (laptop, workstation, Pi, VPS).
---

# Kobold Host Hardening

## Overview

Assess and harden the host running Kobold, then align it to a user-defined risk tolerance without breaking access. Use Kobold security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Kobold changes the host firewall, SSH, or OS updates; it does not.
...
5. Kobold gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Kobold can access sensitive data.
...
### 2) Run Kobold security audits (read-only)

As part of the default read-only checks, run `kobold security audit --deep`. Only offer alternatives if the user requests them:

1. `kobold security audit` (faster, non-probing)
2. `kobold security audit --json` (structured output)

Offer to apply Kobold safe defaults (numbered):

1. `kobold security audit --fix`

Be explicit that `--fix` only tightens Kobold defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Kobold version/update status (read-only)

As part of the default read-only checks, run `kobold update status`.
...
- Credential hygiene notes (location of Kobold creds, prefer disk encryption)
...
After Kobold install or first hardening pass, run at least one baseline audit and version check:

- `kobold security audit`
- `kobold security audit --deep`
- `kobold update status`

Ongoing monitoring is recommended. Use the Kobold cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `kobold cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `kobold update status`
...
Before creating, `kobold cron list` and match on exact `name`. If found, `kobold cron edit <id> ...`.
If not found, `kobold cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `kobold update status` (preferred for source checkouts and channels)
2. `npm view kobold version` (published npm version)

## Kobold command accuracy

Use only supported commands and flags:

- `kobold security audit [--deep] [--fix] [--json]`
- `kobold status` / `kobold status --deep`
- `kobold health --json`
- `kobold update status`
- `kobold cron add|list|runs|run`

Do not invent CLI flags or imply Kobold enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Kobold compaction:
